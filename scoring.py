...
# ── System 2 Deep Analysis scoring ──────────────────────────────────────

S2_COVERAGE_THRESHOLD = get_constant("s2_coverage_threshold", 0.40)


def chunk_transcript(text: str) -> list[str]:
    """Split a System 2 transcript into sentence-level chunks.

    Uses sentence boundaries (period, exclamation, question mark, newline)
    and filters out very short fragments (< 5 chars) that are typically
    meta-talk artifacts.

    Args:
        text: Raw transcript from System 2 free recall.

    Returns:
        List of non-empty sentence chunks, stripped of whitespace.
    """
    import re
...
def score_node_coverage(chunks: list[str], ga_metadata: dict) -> dict[str, float]:
    """Compute cosine similarity between transcript chunks and GA semantic references.

    For each reference text (organized by level: L1_broad, L2_specific, L3_detailed),
    computes the max cosine similarity against all chunks. This tells us which
    knowledge nodes were "received" by the participant during deliberate viewing.

    Args:
        chunks: List of sentence-level chunks from the transcript.
        ga_metadata: GA image metadata containing semantic_references.

    Returns:
        Dict mapping reference_id (e.g. "L1_0", "L2_1", "L3_2") to max cosine
        similarity score. Empty dict if no chunks or no references.
    """
    if not chunks:
        return {}

    sem_refs = ga_metadata.get("semantic_references")
    if not sem_refs or not isinstance(sem_refs, dict):
        return {}

    try:
        from semantic import embed, embed_batch
    except (ImportError, Exception):
        return {}

    # Build flat list of (ref_id, ref_text) pairs
    ref_entries = []
        ...
        if isinstance(level_texts, list):
            for i, text in enumerate(level_texts):
                if isinstance(text, str) and text.strip():
                    ...
                    ref_entries.append((ref_id, text))
    ...
    # Embed all chunks and all reference texts
    ref_texts = [r[1] for r in ref_entries]
    chunk_embs = embed_batch(chunks)     # (N_chunks, dim)
    ref_embs = embed_batch(ref_texts)    # (N_refs, dim)

    # For each reference, find max similarity across all chunks
    # similarity matrix: (N_refs, N_chunks)
    sim_matrix = ref_embs @ chunk_embs.T

    result = {}
    for i, (ref_id, _) in enumerate(ref_entries):
        result[ref_id] = float(np.max(sim_matrix[i]))

    return result


def compute_system2_coverage(node_scores: dict[str, float],
                             threshold: float = None) -> float:
    """Compute overall System 2 coverage: fraction of nodes above threshold.

    Args:
        node_scores: Dict mapping node_id to cosine similarity score.
        threshold: Minimum similarity for a node to count as "received".
            Defaults to s2_coverage_threshold from config.yaml (0.40).

    Returns:
        Float in [0.0, 1.0]: fraction of nodes with similarity >= threshold.
        Returns 0.0 if no nodes.
    """
    if threshold is None:
        threshold = S2_COVERAGE_THRESHOLD
    if not node_scores:
        return 0.0
    n_above = sum(1 for score in node_scores.values() if score >= threshold)
    return n_above / len(node_scores)
